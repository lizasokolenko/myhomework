...
def regexp_0(text: str, pattern: str) -> List[slice]:
    ...
    for n in range(len(text)):
        res = re.match(pattern, text[n:])
        if res:
            list.append(slice(n, res.end() + n))
    return list


def regexp_1(text: str):
    """
...
def regexp_3(text: str) -> str:
    ...
    'Mark'
    """
    reg_patt = re.compile(r'\(([A-z]|[А-я])*\)+')
    text1 = re.sub(reg_patt, '', text)
    text1 = re.sub(' ', '', text1)
    return text1
